  ...
  .post(
...

    {
      detail: {
        ...
        description:
          'Cria uma instância WhatsApp via Evolution API e retorna o QR code para escaneamento',
  ...
  .get(
...

    {
      detail: {
        ...
        description:
          'Retorna a lista de grupos do WhatsApp conectado. Suporta ?force=true para bypass do cache.',
  ...
  .delete(
    ...
    async ({ jwt, request, set }) => {
      ...
      try {
        ...
        const jids = [
          ...new Set(
            mirrors.rows.flatMap((mirror) => (mirror.sourceGroups ?? []).map((group) => group.jid)),
          ),
        ];
        ...
        if (jids.length > 0) {
          ...
          log(
            'info',
            `Cache de sourceGroups limpo para ${instanceName} (${jids.length} grupo(s) removido(s))`,
            { instanceName, count: jids.length },
          );
        }
      } catch {
  ...
  .post(
    ...
    async ({ jwt, request, set }) => {
      ...
      if (!existing) {
        ...
        return {
          success: false,
          error: 'Nenhuma instância WhatsApp encontrada. Conecte-se primeiro.',
        };
      }
...

    {
      detail: {
        ...
        description:
          'Deleta e recria a instância WhatsApp na Evolution API, gerando um novo QR code para escaneamento',
