 ...
 *
 * Porta do `apps/web/src/pages/WorkerStatusPage.tsx` para o admin-web.
 * Diferenças:
 *   - Remove `PageLayout` / `PageHeader` (não existem no admin-web) — usa wrapper
 *     inline baseado em `<div>` com classes utilitárias do `admin-styles.css`.
 *   - Substitui endpoints `/api/worker/*` por `/api/admin/worker/*` (rotas
 *     montadas no admin-api com guard de sessão).
 *   - Adapta o `onBack` para voltar à raiz do admin-web (`/`).
 *   - `useNavigate` continua existindo — o react-router-dom está presente.
 *
 * As seções da página (1. Pipeline, 2. Saúde, 3. Ingestor, 4. Dispatcher,
 * 5. DLQ) ficam preservadas 1:1.
 */
import { useState, useEffect, useCallback, useMemo, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import { Card, Badge, Button, Switch } from '@omestre/ui';
import {
  RefreshCw,
  AlertTriangle,
  ArrowRight,
      ...
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' }}>
            ...
            {mirrorId != null && (
              <span
                title={`Espelhamento #${mirrorId} (use o apps/web para abrir detalhes)`}
                style={{
                  display: 'inline-flex',
                  alignItems: 'center',
                  gap: '0.2rem',
...
function DLQSection() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/worker/dlq?${buildQuery()}`);
  ...
  async function handleRequeue(id: string) {
    setBusyId(id);
    try {
      await fetch(`/api/admin/worker/dlq/requeue?id=${encodeURIComponent(id)}`, { method: 'POST' });
      await load();
    } finally {
      setBusyId(null);
    }
  ...
  async function handleRemove(id: string) {
    setBusyId(id);
    try {
      await fetch(`/api/admin/worker/dlq/remove?id=${encodeURIComponent(id)}`, { method: 'POST' });
      await load();
    } finally {
      setBusyId(null);
    }
  ...
  async function handlePurge() {
    setPurging(true);
    try {
      await fetch('/api/admin/worker/dlq/purge', { method: 'POST' });
      await load();
    } finally {
      setPurging(false);
    }
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <div className="page-title">📊 Status do Worker</div>
          <div className="page-subtitle">Pipeline de espelhamento — Ingestor + Dispatcher</div>
        </div>
        <div className="row">
          {lastUpdate && (
            <span
              title={
                isStale ? 'Dados com mais de 30s — auto-refresh pode estar parado' : 'Dados ao vivo'
              }
              style={{
                fontSize: '0.65rem',
