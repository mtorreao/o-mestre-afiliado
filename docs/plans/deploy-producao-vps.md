...
| `omestreafiliado.com.br` (apex) | 404 — precisa de DNS + Page Rule redirect → `app.`                                                                                                              |
...
| **Como o tunnel expõe o app?**     | Reaproveita tunnel `omestre-tunnel` (já ativo). Adiciona 2 ingress novos: `app.omestreafiliado.com.br → http://127.0.0.1:5441` e `omestreafiliado.com.br → redirect 301 → app.` (via Page Rule do Cloudflare, não no tunnel).                                                                             |
...
### Fase 4 — Cloudflare Tunnel: ingress novos + Page Rule (30min)

Reaproveitar o tunnel `omestre-tunnel` (já ativo). Adicionar 2 hostnames.

1. **Editar `/etc/cloudflared/config.yml`** (no VPS) — adicionar 2 ingress:
   ...
   ingress:
     ...
     # O Mestre Afiliado — Web (nginx) — NOVO
     - hostname: app.omestreafiliado.com.br
       service: http://127.0.0.1:5441
       originRequest:
         connectTimeout: 30s
         noHappyEyeballs: false
     # Apex → 301 para app. — feito via Page Rule do Cloudflare, não aqui
...
2. **Criar/atualizar DNS records no Cloudflare Dashboard** (zona `omestreafiliado.com.br`):
   - `app.omestreafiliado.com.br` → CNAME → `3debf632-b108-46b6-bcaa-ec370dd476db.cfargotunnel.com` (Proxied laranja)
   - Se já existe apontando pra outro tunnel, **sobrescrever** (não deletar — manter backup).

3. **Criar Page Rule no Cloudflare** (free tier permite 3):
   - URL: `omestreafiliado.com.br/*`
   - Status code: **301**
   - Destination: `https://app.omestreafiliado.com.br/$1`
   - ⚠️ Se 3 Page Rules já estiverem em uso, alternativa: Worker de 1 linha grátis (até 100k req/dia). Para começar, Page Rule.
...
4. **Validar tunnel conecta ao app**:
   ...
   # Vai dar 502 até o docker compose up subir, depois 200
   ```

**Critério de aceite:** `cloudflared tunnel info omestre-tunnel` mostra CONNECTOR ID ativo, `curl -I https://app.omestreafiliado.com.br` retorna 502 ou 200 (não 404).
